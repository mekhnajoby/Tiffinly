...
if ($has_active_subscription) {
    $past_sql = "
        SELECT delivery_date, meal_type, status
        FROM delivery_assignments
        WHERE subscription_id = ? AND status IN ('delivered', 'cancelled')
    ...
    <?php endif; ?>
                    ...
                    <div class="delivery-details-card" style="flex: 2; min-width: 300px;">
                        <?php 
                            ...
                            $title = $is_today_view ? "Today's Deliveries" : 'Upcoming Delivery (' . htmlspecialchars($next_delivery_info['date_label'] ?? '') . ')';
                            echo '<h3 class="card-title" style="font-size: 1.2rem; color: #333;"><i class="fas fa-box-open"></i> ' . $title . '</h3>';
                            $display_meals = $is_today_view ? $assigned_meals_today : ($upcoming_meals ?? []);
