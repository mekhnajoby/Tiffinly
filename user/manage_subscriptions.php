...
$subscription = null;
// This query now fetches the most recent subscription that isn't 'cancelled'.
// This includes 'active' subscriptions and 'pending' subscriptions that are unpaid.
$sql = "SELECT s.*, mp.plan_name, mp.plan_type FROM subscriptions s JOIN meal_plans mp ON s.plan_id = mp.plan_id WHERE s.user_id = ? AND s.status != 'cancelled' ORDER BY s.created_at DESC LIMIT 1";
                ...
                <?php if ($subscription): ?>
                    ...
                    <dl class="details-grid">
                        ...
                        <?php if ($subscription['payment_status'] !== 'paid'): ?>
                            <dd style="color: #e74c3c; font-weight: bold;">Unpaid</dd>
                        <?php else: ?>
                            <dd style="color: #28a745; font-weight: bold;"><?php echo ucfirst(htmlspecialchars($subscription['status'])); ?></dd>
                        <?php endif; ?>
