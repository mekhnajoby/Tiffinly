...
if (isset($_GET['action']) && $_GET['action'] === 'remove' && isset($_GET['subscription_id'])) {
    ...
    try {
        // Delete from subscription_meals first
...
if (isset($_GET['message'])) {
    if ($_GET['message'] === 'removed') {
        ...
        $message = 'Could not remove the plan. Please try again.';
    }
}
...
// Fetch meal selections if it is a premium plan
$meals = [];
if ($plan_selection && $plan_selection['plan_type'] === 'premium' && isset($plan_selection['subscription_id'])) {
                ...
                <div id="meal-details-container">
                    ...
                    <table class="cart-table">
                        ...
                        if ($plan_selection['plan_type'] === 'premium') {
                            ...
                            $meal_sql = "SELECT pm.day_of_week, pm.meal_type, m.meal_name, mc.option_type FROM plan_meals pm JOIN meals m ON pm.meal_id = m.meal_id JOIN meal_categories mc ON m.category_id = mc.category_id WHERE pm.plan_id = ?";
