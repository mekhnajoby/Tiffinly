...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_subscription'])) {
    ...
    $status = 'active';
        ...
        // Check for existing pending subscription for same user, plan, and start_date
        $check_sql = "SELECT subscription_id FROM subscriptions WHERE user_id = ? AND plan_id = ? AND start_date = ? AND status = 'pending' LIMIT 1";
        $check_stmt = $conn->prepare($check_sql);
        $check_stmt->bind_param("iis", $user_id, $plan_id, $start_date);
        $check_stmt->execute();
        $check_stmt->store_result();
        if ($check_stmt->num_rows > 0) {
            $check_stmt->bind_result($subscription_id);
            $check_stmt->fetch();
            // Update existing pending subscription details
            $update_sql = "UPDATE subscriptions SET dietary_preference = ?, end_date = ?, schedule = ?, delivery_time = ?, total_price = ? WHERE subscription_id = ?";
            $update_stmt = $conn->prepare($update_sql);
            $update_stmt->bind_param("ssssdi", $dietary_preference, $end_date, $schedule, $delivery_time, $total_price, $subscription_id);
            $update_stmt->execute();
        } else {
...
$meals_sql = "SELECT m.meal_name, mc.category_name FROM meals m JOIN plan_meals pm ON m.meal_id = pm.meal_id JOIN meal_categories mc ON m.category_id = mc.category_id WHERE pm.plan_id = ?";
                ...
                <?php if ($plan_type === 'premium'): ?>
                    ...
                    <form method="POST" action="">
                        ...
                        <div class="meal-customization-grid">
                            ...
                            $plan_meals = [];
                            $plan_id = $plan_selection['plan_id'];
                            $sql = "SELECT pm.day_of_week, pm.meal_type, m.meal_name FROM plan_meals pm JOIN meals m ON pm.meal_id = m.meal_id WHERE pm.plan_id = ?";
                            $stmt = $conn->prepare($sql);
